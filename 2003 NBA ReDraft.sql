...
NormalizedStats AS (
	...
	CASE 
		...
		WHEN Draft_Position <= 10 AND (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN -30 AND -16   THEN 'MAJOR BUST'
		WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) <= -30 THEN 'MAJOR BUST'
		WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN -30 AND -16 THEN 'BUST'
		WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN -15 AND -11 THEN 'LESS THAN EXPECTED VALUE'
        WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN -10 AND 10 THEN 'Expected Value'
		WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN 11 AND 20 THEN 'STEAL'
		WHEN (Draft_Position - ROW_NUMBER() OVER (ORDER BY Career_Impact_Score DESC)) BETWEEN 21 AND 99 THEN 'HUGE STEAL'

    END AS Revised_Value_Status
...
SELECT TOP 58
	...
	-- Revised Value Status Count
	CASE 
        WHEN Draft_Position = 0 AND Redraft_Position <= 30 THEN 'MASSIVE UNDRAFTED STEAL'
        WHEN Draft_Position = 0 AND Redraft_Position <= 60 THEN 'UNDRAFTED STEAL'
		WHEN Draft_Position <= 10 AND (Draft_Position - Redraft_Position) BETWEEN -30 AND -16   THEN 'MAJOR BUST'
		WHEN (Draft_Position - Redraft_Position) <= -30 THEN 'MAJOR BUST'
		WHEN (Draft_Position - Redraft_Position) BETWEEN -30 AND -16 THEN 'BUST'
		WHEN (Draft_Position - Redraft_Position) BETWEEN -15 AND -11 THEN 'LESS THAN EXPECTED VALUE'
        WHEN (Draft_Position - Redraft_Position) BETWEEN -10 AND 10 THEN 'Expected Value'
		WHEN (Draft_Position - Redraft_Position) BETWEEN 11 AND 20 THEN 'STEAL'
		WHEN (Draft_Position - Redraft_Position) BETWEEN 21 AND 99 THEN 'HUGE STEAL'

    END AS Revised_Value_Status,

    Draft_Status,
    ROUND(Career_Impact_Score,0) as Career_Impact_Score,
...
SELECT 
    ...
    ROUND(SUM(Career_Impact_Score),0) AS Total_Impact,
    ROUND(AVG(Career_Impact_Score),0) AS Avg_Impact,
    ROUND(MAX(Career_Impact_Score),0) AS Best_Pick_Impact,
    ROUND(MIN(Career_Impact_Score),0) AS Worst_Pick_Impact,
    
    -- Revised Value Status counts
    COUNT(CASE WHEN Revised_Value_Status = 'MASSIVE UNDRAFTED STEAL' THEN 1 END) AS Massive_Undrafted_Steals,
    COUNT(CASE WHEN Revised_Value_Status = 'UNDRAFTED STEAL' THEN 1 END) AS Undrafted_Steals,
    COUNT(CASE WHEN Revised_Value_Status = 'HUGE STEAL' THEN 1 END) AS Huge_Steals,
    COUNT(CASE WHEN Revised_Value_Status = 'STEAL' THEN 1 END) AS Steals,
	COUNT(CASE WHEN Revised_Value_Status = 'LESS THAN EXPECTED VALUE' THEN 1 END) AS Less_Than_Expected_Value,
	COUNT(CASE WHEN Revised_Value_Status = 'EXPECTED VALUE' THEN 1 END) AS Expected_Value,
    COUNT(CASE WHEN Revised_Value_Status = 'MAJOR BUST' THEN 1 END) AS Major_Busts,
    COUNT(CASE WHEN Revised_Value_Status = 'BUST' THEN 1 END) AS Busts,

	STRING_AGG(Player, ', ') WITHIN GROUP (ORDER BY Normalized_Impact_Score DESC) AS Players,
